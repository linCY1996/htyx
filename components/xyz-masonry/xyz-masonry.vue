	...
	<view>
		<view style="display: flex;justify-content: space-between;" class="masonry ">
			<view style="width: 49%;border-radius: 8px;">
				<view class="card" v-for="(item, index) in list" v-if="index%2==0&&list!=''" :key="index" v-on:tap.stop="toOrder(item.userId)"
				 style="background: white;">
					<image class="imgBase" :src="item.photoList[0]" :mode="mode" style="width: 100%;" lazy-load></image>


					<view style="position: relative;top:-32px;height: 30px;line-height: 30px;display: flex;flex-direction: row;justify-content: space-between;"
					 class="jian">

						<view style="display: inline-block;width: 26px;height: 26px;font-size: 26px;" v-on:click.stop="shou(item.userId)"
						 class='iconfont icon-xinxing iconBase'></view>
						<view v-for="(o,i) in guan" :key='i'>
							<view style="display: inline-block;width: 26px;height: 26px;font-size: 26px;margin-left: -26px;" v-if="o==item.userId"
							 class='iconfont icon-xinxing redxing' v-on:click.stop="cancel(item.userId)"></view>
						</view>
						<label style="width: 100%;"></label>
						<view style="display: inline-block;width: 26px;height: 26px;font-size: 30px;margin-right: 4px;" v-on:click.stop="buy(item.userId)"
						 class='iconfont icon-gouwuche iconBase'></view>
					</view>
					<view style="position: relative;top: -26px;">
						...
						<view style="font-size: 14px;color: #cccccc;display: flex;align-items: flex-start;">
							...
							<view style="display:inline-block;width: 80px;text-align: right;font-size: 20px;font-weight: 600;color: #eb5829;height: 30px; line-height: 30px;">{{item.partTimeMessage.price}}<text
								 style="font-size: 12px;">元/场</text></view>
						</view>
						...
						<view style="color: white;display: flex;align-items: flex-start;flex-wrap: wrap;margin-left: 6px;margin-right: 6px;margin-bottom: 6px;">
							<view v-if="isTag!=''" v-for="(tag,tagIndex) in isTag[index]" :key="tagIndex">
								<view class="TagState" v-if="tag!=''&&tag!=undefined" :style="{'background-color':tagColor[tagIndex]}">{{tag}}</view>
							</view>
						</view>
					</view>
				</view>
			...
			<view style="width: 49%;border-radius: 8px;">
				<view class="card" v-for="(item, index) in list" v-if="index%2!=0&&list!=''" :key="index" v-on:tap.stop="toOrder(item.userId)"
				 style="background: white;">
					<image class="imgBase" :src="item.photoList[0]" :mode="mode" style="width: 100%;" lazy-load></image>
					<view style="position: relative;top:-32px;height: 30px;line-height: 30px;display: flex;justify-content: flex-start;"
					 class="jian">
					...
					<view style="position: relative;top: -26px;">
						...
						<view style="font-size: 14px;color: #cccccc;display: flex;align-items: flex-start;">
							<view style="display:inline-block;width: 50px;text-align: center;height: 30px; line-height: 30px;color: #f09a38;">{{item.partTimeMessage.height}}CM</view>
							<view style="display:inline-block;width: 50px;text-align: center;height: 30px; line-height: 30px;color: #f09a38;">{{item.partTimeMessage.weight}}KG</view>
							<view style="display:inline-block;width: 80px;text-align: right;font-size: 20px;font-weight: 600;color: #eb5829;height: 30px; line-height: 30px;">{{item.partTimeMessage.price}}
								<text style="font-size: 12px;">元/场</text></view>
						</view>
						<view style="color: white;display: flex;align-items: flex-start;flex-wrap: wrap;margin-left: 6px;margin-right: 6px;margin-bottom: 6px;">
							<view v-if="isTag!=''" v-for="(tag,tagIndex) in isTag[index]" :key="tagIndex">
								<view class="TagState" v-if="tag!=''&&tag!=undefined" :style="{'background-color':tagColor[tagIndex]}" >{{tag}}</view>
