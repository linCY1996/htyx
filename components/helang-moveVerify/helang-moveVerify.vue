<template>
	<view class="pathway" @touchend="onEnd" id="pathway">
		<view class="tips">
			<text v-if="isOk" style="color: #FFFFFF;">抢单成功</text>
			<text v-else>滑动抢单</text>
		</view>
		<view class="track" :style="{'transform':'translateX('+isOldx+'px)'}"></view>
		<movable-area :animation="true">
			<movable-view id="track" :class="{'active':isOk}" :x="isX" direction="horizontal" @change="onMove"><text style="display: none;">{{isX}}</text></movable-view>
		</movable-area>
		<view class="disabled" v-if="setOk"></view>
	</view>
</template>
